<x-app-layout>
  ...
  <div class="p-4 bg-white rounded-lg shadow-xs">
    <h1 class="text-2xl font-semibold text-gray-800">Edit Penginapan</h1>

    <div class="overflow-hidden mb-8 w-full rounded-lg shadow-xs">
      <div class="overflow-x-auto w-full">
        <form action="{{ route('penginapan.update', $penginapan->id) }}" method="POST" autocomplete="off"
          ...
          <div class="mt-4">
            ...
            <x-text-input type="text" id="location" name="location" class="block w-full"
              value="{{ old('location', $penginapan->location) }}" required />
          ...
          <div class="mt-4">
            ...
            @if ($penginapan->photo)
              <div class="mt-2">
                <img src="{{ asset('storage/' . $penginapan->photo) }}" alt="Current Photo"
          ...
          <div class="mt-4">
            ...
            <x-text-input type="number" id="price" name="price" class="block w-full"
              value="{{ old('price', $penginapan->price) }}" required />
          ...
          <div class="mt-4">
            ...
            <select name="status" id="status"
              ...
              required>
              <option value="active" {{ old('status', $penginapan->status) == 'active' ? 'selected' : '' }}>Tersedia
              </option>
              <option value="inactive" {{ old('status', $penginapan->status) == 'inactive' ? 'selected' : '' }}>Tidak
                Tersedia</option>
            </select>
